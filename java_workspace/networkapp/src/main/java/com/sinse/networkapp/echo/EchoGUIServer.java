...
public class EchoGUIServer extends JFrame{
	...
	public void runServer() {
		...
		try {
			...
			Socket socket = server.accept(); // 접속자 기다리기, 접속이 발견되면 소켓 반환
	...
	// 우리가 실행부라고 알고있었던 존재가 사실 메인 쓰레드라 불리는 프로그램 운영 쓰레드이다.
	// 아이폰, 안드로이드폰,, 기타,,프로그램에서 프로그램을 운영하는 메인쓰레드
	// 1) 무한루프, 2) 대기상태 accept(), read()
	// 메인 쓰레드를 대기 상태 빠트리면, 이벤트 감지, GUI 그래픽 처리 불가... 모든 게 다 멈춘다..
