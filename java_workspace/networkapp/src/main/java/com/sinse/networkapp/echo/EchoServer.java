...
public class EchoServer {
	...
	public EchoServer() {
		try {
			...
			// 소켓을 통해서 데이터를 주고 받을 수 있는 스트림을 얻자
			// 방향에 따라 - 입력, 출력
			// 데이터 처리 방식 - 바이트 ~~Stream, 문자 ~~Reader, Writer 버퍼 
			InputStream is = socket.getInputStream(); // 바이트기반의 입력스트림을 얻어옴
			OutputStream os = socket.getOutputStream(); // 바이트기반의 입력스트림을 얻어옴
			
			// 무한루프는 엄청난 속도이므로, 프로그램에서 사용 시 주의해야 하지만,
			// 단, 스트림 처리에서는 read() 메서드 자체가 상대방의 메시지를 받을 때 가지 대기상태에 빠지기
			// 때문에 부하를 일으키지 않음
			while(true) {
				int data = is.read(); // 1byte 읽어들임.. yahoo중 y만
				System.out.print((char)data);
				os.write(data); // 읽어들인 1바이트 데이터를 그대로 보내버림
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
