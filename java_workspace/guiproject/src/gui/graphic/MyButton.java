...
따라서 개발자가 원하는 그림으로 커스텀 하려면, 그 그림을 뻇어서 그리면
...
public class MyButton extends JButton {
	...
	// 따라서 super() 호출하지 않고, 매개변수 있는 생성자를 호출
	public MyButton(String title){
		// int x = 3; 부모의 초기화보다 앞서는 코드는 존재할 수 없다
		super(title);
	}
	
	// 버튼 뿐 아니라 컴포넌트들이 보유하고 있는 paint() 메서드를 오버라이딩 하기
	// Graphics : 그림 그리는 도구 (도형, 이미지, 글씨 , 선 ,점)
	// 버튼 그림 뺏기 결론 : 컴포넌트 중 본래의 그래픽을 사용해야 할 경우,
	// 개발자가 그림을 뻇어야 할 상황이 있음.
	public void paint(Graphics g){
			System.out.println("버튼의 그림을 뺐었어요");
