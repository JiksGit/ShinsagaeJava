...
public class ImageUtil {
	...
	public Icon getIcon(String filename, int width, int height) {
		...
		URL url = myClass.getClassLoader().getResource(filename); // 패키지이어야 getClassLoader가 찾을 수 있음
		
		// ImageIcon 자체는 크기를 지정할 수 없음, 따라서 크기 조절이 가능한
		// 이미지를 먼저 얻어야 함
		// Toolkit 을 통해 Image 객체는 픽셀 정보까지 접근할 수 없기 때문에 단순 이미지를 보여줄 때
		// 이용 가치가 있음 .. 회사의 이미지 로고를 박은 워터마크 처리 등의 보다 섬세하고 기술적인 이미지 제어를
		// 하려면, BufferedImage 필요
