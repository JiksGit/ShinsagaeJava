...
javascript의 실행 위치 - 클라이언트의 웹브라우저에서 실행됨
 ...
 -->
<%
// 장차, 미래에 이 jsp가 서블릿으로 변경될 때, service() 메서드의 영역이 된다.
// 따라서 개발자는 스크립틀릿 영역에 요청을 처리하는 코드를 작성하면 된다.
connect(); // 접속 
// 아래의 변수 out은 개발자가 현재 코드에서 선언한 적이 없음에도 불가하고 그 기능을 제대로 한다
// 개발자가 정의하지 않아도 Tomcat 컨테이너가 jsp내에서 기본적으로 사용할 수 있도록
// 미리 생성해놓고 개발에 사용될 수 있도록 지원하는 jsp 의 객체들을 가리켜 내장객체라 한다
// built-in object
// out은 문자기반의 출력 스트림이다!
