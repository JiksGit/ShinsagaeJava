...
<%
	// 이 서블릿에 요청을 시도할 때, 웹 컨테이너에서는 세션 객체를 생성하면서, 동시에 세션ID가 할당되어짐
	// 그리고 이 세션 아이디는 톰캣이 보내는 응답정보에 쿠키형태로 존재하게 됨
	// 단 세션과 세션 아이디는 매 요청마다 만들어지는게 아니라, 클라이언트가 요청 시 톰캣이 발급한 쿠키가
	// 존재하지 않을 때는 새로 만들고, 요청 후 일정 시간동안 아무런 재요청이 없을 때는 사용하지 않는 것으로
	// 판단하여 새롭게 세션 할당
