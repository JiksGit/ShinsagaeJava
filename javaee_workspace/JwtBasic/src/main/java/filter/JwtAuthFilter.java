...
//코드가 중복되므로, 필터 수준에서 jwt 보유 여부를 검증하고, 즉 로그인 검증을 처리하고 이 시점에
...
public class JwtAuthFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(authHeader==null || !authHeader.startsWith("Bearer ")) {
			//클라이언트에게 jwt가 없다는 에러 메시지 전송..
			res.setContentType("application/json;charset=utf-8");
			res.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
			
			//아래와 같이 문자열로 json을 표기하면 피곤할 수 있으므로, Gson을 이용하여 java 객체를 json문자열로
			//바꿔도 된다
			//res.getWriter().print("{ \"error\":\"말\" }");
		...
		//토큰을 가져온 경우 , 순수 토큰값을 꺼내서 검증.. 
		
		//Bearer ewyrshdfjsdhkajhf   결국 index 7번부터 모든 문자열 substring(7, 문자열.length), substirng(7)
		...
		//인증에 성공한 회원에게는 username를 사용할수있는 기회를 제공 
