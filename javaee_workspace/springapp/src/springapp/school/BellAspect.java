 ...
 * 
 * 스피링이 개발되기 이전부터 IT분야에 있던 개념임.. 그리고 자바 기반의 AOP 라이브러리도
...
// (언제 올려지지?) -> AppMain에서 설정할때
// AppConfig에서 Bean으로 등록하지 못하므로 ComponentScan을 통해 싱글톤으로 메모리를 올린다
public class BellAspect {
	
	@Autowired // 자동 주입(스프링 컨테이너가 보유한 Bell 빈을 아래 멤버변수에 자동으로 주입) 
	private Bell bell; // 공통 코드, 횡단점 관심사
