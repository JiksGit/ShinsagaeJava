...
public class FoodTypeDAO {
	...
	public List selectAll() throws FoodTypeException{
		...
		/* java 7부터 try~with~resource 문법이 지원
		 * 자원을 사용한 후 finally에서 닫는 중복 코드를 단순화 시키기 위함
		 * 즉 언어 차원에서 개발자가 해야할 close()메서드 호출을 자동으로 해줌
		 * 주의) 모든 close()메서드를 대상으로 하지않고, Closeable 을 구현한 객체만을 대상으로함
		...
		try(Session session = config.getSession()){
			...
			// 보이지만, from 절에 대상이 되는 객체는 테이블이 아니라 클래스다
