...
public class FoodTypeList extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 클라이언트가 원하는 컨텐츠가 html 문서가 아니라, html문서에 들어갈 일부 데이터인 경우
		// 개발자는 순수 데이터를 구성해야 한다
		PrintWriter out = response.getWriter();
		response.setContentType("application/json;charset=utf-8");
		
		// 클라이언트에게 json 형식의 데이터를 전송할 때, 문자열로 변환해야 하지만, 처리할
		// 문자열이 너무 복잡하여 효율성이 떨어짐... java 객체와 json 문자열과의 변환을 자동으로
		// 처리해주는 구글이 제공하는 라이브러리가 있음 => GSON
		Gson gson = new Gson();
		String result = gson.toJson(list); // java.util.List를 json 문자열로 변환, 문자열이기 때문에 전송이 가능
		
		logger.debug("문자열로 변환 후 데이터 " + result);
		out.print(result); // 저장해놓으면, Tomcat이 응답 컨텐츠로 만들어서 보낸다
