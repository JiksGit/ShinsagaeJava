...
/*
useState는 react의 데이터 상태(state) 훅(hook)으로 BoardList 컴포넌트 안에서 상태 관리를 담당
boards: 현재의 상태값
setBoards:상태를 변경할수 있는 함수(setter를 통해서만 제어가능)
*/
    ...
    1) 매 랜더링마다(이 컴포넌트가 화면에 그려질 때마다) - 거의 안씀
        userEffect()에 매개변수로 배열이 존재하지 않을 경우
    2) 단 1회만 호출되는 경우
    ...
    // 서버와의 통신을 비동기로 진행
    // async로 선언된 함수는 반드시 내부에 await 명시해야 함
    // await가 명시된 함수 호출은, 해당 함수 호출이 완료되어야 그 다음 코드가 수행됨.. 즉 일종의
    // 동기 방식의 수행됨..(순서 지킴)
    const load=async()=>{
        const res = await getBoards(); // 서버로부터 가져온 json 배열 res에 보관
        console.log("목록을 가져왔습니다.", res);
        setBoards(res.data);
    }
    return (
