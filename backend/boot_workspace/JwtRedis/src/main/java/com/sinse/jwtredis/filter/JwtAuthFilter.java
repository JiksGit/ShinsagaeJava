...
/*
    ...
    1) 클라이언트의 헤더에 들어있는 Authorization 에서 Bearer 토큰 추출
    2) JWT 서명 조작여부/만료 기간 검증 (유효한 토큰인지 조사)
    3) 블랙리스트 여부 검증(이미 로그아웃한 유저인 경우 401로 전송)
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
           ...
           /* -----------------------------------------------
           2) JWT 파싱 및 서명 조작 여부, 기간만료 등을 검증
           ------------------------------------------------- */
           ...
           ------------------------------------------------- */
               ...
               // Redis에서 블랙리스트 조사
                ...
                int currentVer = redis.currentUserVersion(userId);
                if( currentVer != ver ){
                    // 버전 불일치로 인한 401 처리
                    response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                    return;
            ...
            // 요청 가로막았으므로, 다시 요청의 흐름을 정상화 시킴
            filterChain.doFilter(request, response);

        } catch(ExpiredJwtException expiredJwtException) {
            // 만료된 토큰인 경우...
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            response.getWriter().write("ERR!! : token expired");
