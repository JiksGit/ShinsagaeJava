...
/*
    ...
    1) 로그아웃 한 유저를 어떤 방식으로 블랙리스트에 저장할 것인지 키에 대한 설계
        bl:access:<Jti>
    2) 사용자의 모든 디바이스로부터 로그아웃을 수행하기 위한 버전 값에 대한 설계
        uv:<userId>
...
public class RedisTokenService {
    ...
    참고) 버전을 사용하는 이유?
            JWT + Redis 환경에서 로그아웃 처리시 블랙리스트만으로는 충분하지 않을 때가 있음
            예를 들어, 회원이 로그아웃 하거나 비밀번호를 바꿨을 때, 기존에 발급된 AccessToken 및
            RefreshToken은 모두 무효화 해야함
            이때, 토큰 페이로드에 ver:1 과 같은 사용자버전을 넣어두고, Redis에서 "uv:유저id=2"
            로 저장해놓으면 토큰의 페이로드에 갖고 있는 버전1은 Redis보다 낮으므로, 낮은 버전을
            가진 토큰은 전부 무효화 처리할 수 있으므로
            [ 블랙리스트와 차이점 ]
            블랙리스트 방식 - 토큰 단위로 차단하는 방식, 만료될 때 까지 redis에 개별 키 보관
            버전방식 - 사용자 단위로 이 사용자가 사용중인 모든 디바이스까지 한꺼번에 토큰을 무효환
     */
    public int currentUserVersion(String userId) {
        ...
        // StringRedisTemplate 객체는 redis가 반환한 nil을 java의 null로 변환
        return (v==null) ? 0 : Integer.parseInt(v);
    }

    // AccessToken을 블랙리스트 등록
    // ttlSeconds 토큰에 남은 잔여 시간
    public void registBlackList(String jti, long ttlSeconds) {
        // ttlSeconds이 이미 만료된 시간인 경우엔 블랙리스트에 등록할 필요조차없다
