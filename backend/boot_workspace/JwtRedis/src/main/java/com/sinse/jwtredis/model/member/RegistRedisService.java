...
public class RegistRedisService {
    ...
    // TTL(Time To Live) 설정
    private static final Duration PENDING_TTL =  Duration.ofMinutes(3); // 임시가입 인증 만료 시간 3분

    // 키 설계 (redis에 사용될 키 규칙- 개발자가 정하는 것임)
    private String pendingKey(String email){
        return "pending:"+email; // pending:zino1234@naver.com
    }
    ...
    public RegistRedisService(StringRedisTemplate redis) {
        // redis에게 데이터를 넣을 때 문자열화 시켜서 넣어야 하므로, 그 문자열 작업을
        // 쉽게 처리해주는 라이브러리
        this.redis = redis;
    }
    ...
    public String generateCode6() {
        return String.format("%06d", random.nextInt(1_000_000)); // 6자리 랜덤값 0 ~ 999,999

    }

    // 임시 가입 (redist에 쓰기)
    public void savePending(MemberDTO memberDTO) {
        // Member 라는 java객체가 redis 로 insert 되려면, 문자열화 되어야 함
        try {
            String json = objectMapper.writeValueAsString(memberDTO);

            // 회원 임시 정보 , hset pending:email, json, 15
            redis.opsForValue().set(pendingKey(memberDTO.getEmail()), json, PENDING_TTL);

            // 이메일을 찾을 수 있도록 index를 생성하자, sadd code:mailList email ,15 s
