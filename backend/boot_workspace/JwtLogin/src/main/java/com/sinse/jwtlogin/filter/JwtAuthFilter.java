...
// 단, javaEE의 일반적인 필터가 아니라, Spring Security에서 지원되는 필터를 개발자가 커스텀
// 예를 들어, DispatcherServlet 앞뒤에 여러 필터 체인이 있더라도, 같은 요청에 대해 이 필터가 여러번 호출
// 되지 않도록 보장함
// JWT 토큰 검증 시, 검증은 한 번만 실행되면 충분한 작업이므로..
// 이 객체는 필수인가? -> 선택이지만, 만일 필터를 사용하지 않을 경우, 모든 컨트롤러의 메서드마다
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        log.debug("OncePerRequestFilter로 구현한 검증 필터 동작함");

        // 클라이언트가 Authorization의 Bearer 와 함께보낸 Token을 검증
        String header = request.getHeader("Authorization");

        if (header != null && header.startsWith("Bearer ")) {
            String token = header.substring(7); // index 7번째부터 토큰이 시작되므로

            if(jwtUtil.validateToken(token)){// 토큰이 유효하다면
                // 사용자 정보 중 username 추출
                String username =  jwtUtil.getUsername(token);
                log.debug("토큰으로부터 추출한 username은 : " + username);
                // Security에게 이 요청이 인증을 받은 요청이라는 사실을 저장해야한다
