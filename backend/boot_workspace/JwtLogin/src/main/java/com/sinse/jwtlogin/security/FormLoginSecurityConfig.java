...
// 스프링 시큐리티를 개발자가 원하는 설정으로 변경하고자할 때 이 클래스 정의가 필요함
//@Configuration
//@EnableWebSecurity
public class FormLoginSecurityConfig {
    ...
    public PasswordEncoder passwordEncoder() {
        return new BCryptPasswordEncoder(); // 비번결과물 자체에 Salt가 포함되는 알고리즘
    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                // 스프링 시큐리티가 기본적으로 CSRF를 활성화, 아래의 코드로 disable 시키면
                // CSRF를 비활성화시킴 - 전통적인 form 로그인/세션 방식이 아니라 REST/JWT 로 인증을 처리할 때는
                // 보통 끔
                .csrf(AbstractHttpConfigurer::disable)
                .formLogin(form -> form
                        .loginPage("/member/login.html") // 로그인폼의 접근 url
                        .loginProcessingUrl("/member/login") // 시큐리티가 로그인을 처리할 url

                        // 동기 방식으로 요청이 들어올 때, 결과를 html 보여줄 때 아래의 코드를 사용
